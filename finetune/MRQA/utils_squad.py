...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    features = []
    for (example_index, example) in enumerate((examples)): # tqdm
        ...
        if cls_token == '[CLS]':
            for (i, token) in enumerate(example.doc_tokens):
                ...
                if token=='[PAR]':
                    sub_tokens = ['[unused201]']
                elif token=='[TLE]':
                    sub_tokens = ['[unused202]']
                elif token=='[DOC]':
                    sub_tokens = ['[unused203]']
                else:
            ...
            for tok in example.doc_tokens:
                if tok not in ['[PAR]', '[TLE]', '[DOC]']:
                    jtokens.append(tok)
                else:
                    jtokens.append('<s>')

            jtokens_text = ' '.join(jtokens)
            subwords = tokenizer.tokenize(jtokens_text)
            L = 0
            sub_idx = 0
            flag = 0
            while sub_idx < len(subwords):
                subword_list = []
                while sub_idx < len(subwords):
                    ...
                    if flag == 0:
                        if jtokens[L] == subword:
                            break
                    else:
                        if jtokens[L] == subword[1: ]:
                            break
                if flag == 0:
                    flag = 1
                    assert(jtokens[L] == subword)
                else:
                    assert(jtokens[L] == subword[1: ] or jtokens[L] == '<s>')
                orig_to_tok_index.append(len(all_doc_tokens))
                for s_word in subword_list:
                    tok_to_orig_index.append(L)
                    all_doc_tokens.append(s_word)
                L += 1
