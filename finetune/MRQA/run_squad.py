...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            #     xxx = batch[0][0].cpu().numpy().tolist()
            #     idx2token = {v: k for k,v in tokenizer.vocab.items()}
            #     logger.info(' '.join([idx2token[f] for f in xxx]))
            #     logger.info('\n')
            # elif args.model_type == 'roberta':
            #     xxx = batch[0][0].cpu().numpy().tolist()
            #     logger.info(' '.join([tokenizer.decode(f) for f in xxx]))
            #     logger.info('\n')
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and (global_step % args.save_steps == 0):
                    ...
                    if _>0 and args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
...
def main():
    ...
    if args.ckpt_to_load != "None":
        ...
        if os.path.exists('***path_to_your_trained_checkpoint***'):
            ckpt = torch.load("***path_to_your_trained_checkpoint***"+args.ckpt_to_load)
        else:
            ckpt = torch.load("***path_to_your_trained_checkpoint***"+args.ckpt_to_load)
        if args.model_type == 'bert':
